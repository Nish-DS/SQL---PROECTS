...
-- 1)LIST ALL STATES IN WHICH WE HAVE CUSTOMERS WHO HAVE BOUGHT CELLPHONES 2005 OWNWARDS;
...
SELECT T4.State, COUNT(T3.IDCustomer) AS NO_OF_BUYERS
FROM DIM_MANUFACTURER AS T1 
INNER JOIN DIM_MODEL AS T2 ON T1.IDManufacturer =T2.IDManufacturer
INNER JOIN FACT_TRANSACTIONS AS T3 ON T3.IDModel = T2.IDModel
INNER JOIN DIM_LOCATION AS T4 ON T4.IDLocation = T3.IDLocation
WHERE T1.Manufacturer_Name = 'Samsung' AND T4.Country = 'US'
GROUP BY T4.State) AS T5
ORDER BY NO_OF_BUYERS DESC;

--3) SHOW THE NUMBER OF TRANSACTIONS FOR EACH MODEL PER ZIP MODEL PER STATE
SELECT T2.Model_Name, T3.ZipCode, T3.State, COUNT(IDCustomer) AS NO_OF_TRANSACTIONS
FROM FACT_TRANSACTIONS AS T1 
INNER JOIN DIM_MODEL AS T2 ON T1.IDModel = T2.IDModel
INNER JOIN DIM_LOCATION AS T3 ON T1.IDLocation = T3.IDLocation
GROUP BY T2.Model_Name, T3.ZipCode, T3.State

--4) SHOW THE CHEAPEST CELL PHONE
SELECT  TOP 1 Model_Name  FROM DIM_MODEL
ORDER BY Unit_price

--5) AVERAGE PRICE FOR EACH MODEL IN TOP 5 MANUFACTURERS IN TERMS OF SALES QUANTITY AND ORDER BY AVERAGE PRICE
...
--6) NAMES OF CUSTOMER AND AVERAGE AMOUNT SPENT IN 2009, WHERE THE AVERAGE IS GREATER THAM 500
...
HAVING AVG(TotalPrice) > '500'
 ...
 --7) MODEL THAT WAS IN TOP 5 IN TERMS OF QUANTITY, SIMUNTANEOUSLY IN 2008,2009,2010
SELECT IDModel, Model_Name, Manufacturer_Name 
FROM DIM_MANUFACTURER AS TBL1 INNER JOIN 
DIM_MODEL AS TBL2 ON TBL1.IDManufacturer = TBL2.IDManufacturer
...
GROUP BY IDModel
ORDER BY SUM(Quantity) DESC) AS TBL3
INTERSECT 
SELECT IDModel FROM (
...
WHERE DATEPART(YEAR,Date) = 2009 
GROUP BY IDModel 
ORDER BY SUM(Quantity) DESC) AS TBL4
INTERSECT 
SELECT IDModel FROM (
SELECT TOP 5 IDModel,
SUM(Quantity) AS TOTAL_QTY
...
GROUP BY IDModel
ORDER BY SUM(Quantity) DESC) AS TBL5)

--8) MANUFACTURER WITH 2nd SALES TOP IN YEAR 2009 AND MANUFACTURER WITH 2nd TOP SALES IN YEAR 2010
SELECT * FROM (
SELECT
ROW_NUMBER() OVER ( PARTITION BY DATEPART(YEAR,Date) ORDER BY SUM(Quantity) DESC) AS  RNUM,
 T2.IDModel , T3.IDManufacturer, Manufacturer_Name, DATEPART(YEAR,Date) AS DATE_YEAR, SUM(Quantity) AS TOTAL_SALES 
 FROM FACT_TRANSACTIONS AS T2 LEFT JOIN DIM_MODEL AS T3
 ON T2.IDModel = T3.IDModel LEFT JOIN DIM_MANUFACTURER AS T4 
 ON T3.IDManufacturer = T4.IDManufacturer
WHERE DATEPART(YEAR,Date) IN ('2009', '2010')
GROUP BY T2.IDModel, DATEPART(YEAR,Date), T3.IDManufacturer, Manufacturer_Name
) AS T1
WHERE RNUM = 2

--9) MANUFACTURES THAT HAVE SOLD CELLPHONE IN 2010 AND NOT IN 2009
SELECT Manufacturer_Name FROM DIM_MANUFACTURER WHERE IDManufacturer IN (
SELECT IDManufacturer FROM DIM_MODEL WHERE IDModel IN (
SELECT IDModel FROM FACT_TRANSACTIONS
WHERE DATEPART(YEAR,DATE) = '2010'
EXCEPT
SELECT IDModel FROM FACT_TRANSACTIONS
WHERE DATEPART(YEAR,DATE) = '2009'))

--10)Find top 100 customers  and their average spend, average quantity by each year. Also find percentage of change in their spend.
SELECT TOP 100 *,
CONCAT(CAST(ROUND(COALESCE(((TABLE1.AVG_TRANSACTIONS - (LAG(TABLE1.AVG_TRANSACTIONS,1)OVER(PARTITION BY IDCustomer ORDER BY YEAR_OF_TRANSACTION)))*1.0/TABLE1.AVG_TRANSACTIONS),0)*100.00,2)
AS FLOAT),'%') AS YOY_CHANGE FROM 
(SELECT ROW_NUMBER() OVER (PARTITION BY A.IDCustomer ORDER BY DATEPART(YEAR,Date)) AS ROW_NUM,
 A.IDCustomer, Customer_Name,
 AVG(TotalPrice) AS AVG_TRANSACTIONS, AVG(Quantity) AS AVG_QUANTITY, 
 DATEPART(YEAR,Date) AS YEAR_OF_TRANSACTION FROM DIM_CUSTOMER AS A
 INNER JOIN FACT_TRANSACTIONS AS B ON A.IDCustomer = B.IDCustomer
 GROUP BY 
 DATEPART(YEAR,Date), A.IDCustomer, Customer_Name) AS TABLE1
